...
| Body | `body` | always | `$article['body']` (rich-text HTML emitted by the Tiptap editor) **or** `$article['source_file']` (referenced .html file) — picked by `body_mode` | Phase 20.3: three body-source modes. `rtf` echoes `$article['body']` inside `.article-prose`. `html-body` keeps the surrounding chrome but `readfile()`s a file in `/content/<type>/<slug>/` into the same `.article-prose` slot. `html-swap` bypasses the whole template and renders the file as the full document (no chrome, no nav). Inline images (Figure) accept JPEG/PNG/WebP/GIF/SVG. Figure also carries two independent toggles, `data-rounded`/`data-border` (both implicit-on by default, "0" = off) — a right-side pill pair in the same floating panel as the size pills. At `data-size="full"`, rounded is always forced off and the border (if on) only shows top/bottom, never left/right — the image bleeds to the viewport edge, so there's no edge to put a side border against. Articles' toolbar also has an HTML Embed block (`div.html-embed`, atom node) for hand-pasted markup (e.g. an exported SVG) — same size toggle (default/wide/full) as Figure, no rounded/border toggle (nothing to toggle — the block has no border styling), and its subtree is the one deliberate exception to `lib/sanitize.php`'s allowlist (passes through untouched; single-author site). Journals/live sessions/experiments don't have inline image or embed insertion in their toolbars (pre-existing scope, unchanged here). Both Articles' and Experiments' toolbars have an "H2^" button that toggles the current block to/from a section-header kicker (`p.kicker`) — same toggleNode pattern the H2/H3 buttons use, just toggling to `kicker` instead of `heading`. A small uppercase label for a repeating in-body section header, distinct from the listing/hero eyebrow variants elsewhere in the design system. A plain block node (not an atom), and not required to pair with an H2 — but if one immediately follows a kicker, `.article-prose .kicker + h2` zeroes that H2's own top margin so the pair reads as one unit. Not category-tinted — uses the body text colour regardless of the content's category. |
