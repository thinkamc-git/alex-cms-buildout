 ...
 *
 * Two zones of nav-item rowforms (header / footer) + a Layout partials
 * section that links to the Pages editor for template editing.
 */
...
// ── Layout partials data (for the templates section) ──────────────────────────
$partials_data = [];
foreach (['header', 'footer'] as $slug) {
    $file  = find_page_file($slug);
    $mocks = list_page_mocks($slug);
    $partials_data[$slug] = [
        'slug'         => $slug,
        'label'        => ucfirst($slug) . '.php',
        'edit_url'     => '/cms/pages/edit?slug=' . rawurlencode($slug) . '&from=navigation',
        'draft_saved'  => !empty($mocks[0]['updated_at']) ? (int)strtotime((string)$mocks[0]['updated_at']) : 0,
        'published_at' => $file ? (int)$file['modified_at'] : 0,
    ];
}
...
<style>
  ...
  .np-mark .np-dot { width:14px; height:14px; border-radius:50%; background:var(--np-color,#d63031); display:none; }
  .np-mark.hl-dot .np-dot   { display:inline-block; }
  .np-mark.hl-dot input     { display:none; }
  .np-mark.hl-pill input {
    background:var(--np-color,#d63031); color:var(--np-contrast,#fff);
    border-color:transparent; font-family:var(--font-cond); font-size:10px;
    font-weight:700; letter-spacing:0.08em; text-transform:uppercase;
    text-align:center; padding:4px 8px; border-radius:10px;
    field-sizing:content; width:auto; min-width:40px; max-width:100%;
  }
  ...
  .rowform-row .np-mark.hl-pill input:focus {
    background:var(--np-color,#d63031); border-color:transparent;
  }
  .rowform-row .np-mark.hl-pill input:focus {
    outline:none; box-shadow:inset 0 0 0 1px rgba(255,255,255,0.55);
  }
  .np-mark.hl-none input { visibility:hidden; }
  .pill-broken {
    display:inline-block; font-family:var(--font-cond); font-size:9px;
    letter-spacing:0.08em; text-transform:uppercase; padding:2px 6px;
    border-radius:3px; background:var(--c-terracotta); color:white;
...
<div class="layout">
  ...
  <main class="main" id="main" tabindex="-1">
    <div class="view active" id="view-navigation">
      <?php
      $title    = 'Navigation';
      $subtitle = 'Manage nav links and the header / footer templates.';
      ...
      <div class="content-area">
        ...
        $renderZone = function (string $zone, array $items) use ($e, $csrf_token, $broken_ids,
            $indexes, $categories, $series_rows, $content_rows, $renderMarkCell): void {
                ...
                <div class="rowform-row nav-row<?= $broken ? ' is-broken' : '' ?>" draggable="true" data-id="<?= (int)$it['id'] ?>">
                  ...
                  <form method="post" action="/cms/navigation" id="nav-form-<?= (int)$it['id'] ?>" style="display:contents">
                    ...
                    <span class="nav-picker">
                      ...
                      <input type="text" name="target_slug" value="<?= $e((string)$it['target_slug']) ?>" placeholder="page-slug" data-picker="page" style="<?= $it['target_type']==='page'?'':'display:none' ?>">
                    ...
                    <label class="ds-check" title="Show on mobile — uncheck to hide" style="justify-self:center">
        ...
        <div class="content-block">
          <div class="content-block-header">
            <div>
              <span class="content-block-label">Templates</span>
              <span class="content-block-sublabel">Header and footer templates that frame every public page</span>
            </div>
            <span class="content-block-count">2 files</span>
          </div>

          <?php
          $columns = [
              ['label' => 'Template',       'width' => '40%'],
              ['label' => 'Draft saved',    'width' => '20%'],
              ['label' => 'Last published', 'width' => '20%'],
              ['label' => '',               'width' => '20%'],
          ];
          $rows = [];
          foreach ($partials_data as $p) {
              $draft_html = $p['draft_saved'] > 0
                  ? rel_time_html($p['draft_saved'])
                  : '<span class="muted">—</span>';
              $pub_html = $p['published_at'] > 0
                  ? rel_time_html($p['published_at'])
                  : '<span class="muted">—</span>';
              $actions_html = '<div class="row-actions row-actions-always">'
                  . '<a href="' . $e($p['edit_url']) . '" class="btn-sec btn-tiny">Edit →</a>'
                  . '</div>';
              $rows[] = [
                  'href'  => $p['edit_url'],
                  'cells' => [
                      ['html' => '<a href="' . $e($p['edit_url']) . '" class="row-title">' . $e($p['label']) . '</a>'],
                      ['html' => $draft_html],
                      ['html' => $pub_html],
                      ['html' => $actions_html, 'class' => 'cell-actions'],
                  ],
              ];
          }
...
<script>
  ...
  function navHighlightChanged(select) {
    navRefreshMark(select.closest('form'));
  }
  ...
  const navSupportsFieldSizing = (() => { try { return CSS.supports&&CSS.supports('field-sizing','content'); } catch(_){return false;} })();
  ...
  document.querySelectorAll('.nav-row form').forEach(f => {
    ['select[name=highlight]','input[name=highlight_color]','input[name=highlight_text]'].forEach(sel => {
      ...
      if (!el) return;
      el.addEventListener(el.tagName==='SELECT'?'change':'input', ()=>navRefreshMark(f));
    });
  });
