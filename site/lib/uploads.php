...
function accept_upload(array $file, string $subdir): array
{
    ...
    // Defense-in-depth: ensure the uploads root can never execute an uploaded
    // file as code (belt-and-braces against a future Apache misconfig). The
    // uploads/ tree is runtime-only — excluded from deploy — so we self-heal
    // the guard here rather than shipping it.
    upload_ensure_noexec_guard($webroot . '/uploads');
...
/**
 * Write (once) a hardening .htaccess at the uploads root that forbids the
 * server from ever executing an uploaded file as code. Idempotent and cheap;
 * skipped if the guard already exists.
 */
function upload_ensure_noexec_guard(string $uploadsRoot): void
{
    ...
    $rules = <<<HTACCESS
        # Auto-generated by lib/uploads.php — do NOT execute uploaded files as
        # code. Uploads are user-supplied data, served as static assets only.
        <IfModule mod_php.c>
          php_admin_flag engine off
        </IfModule>
        RemoveHandler .php .phtml .php3 .php4 .php5 .php7 .php8 .pht .phar
        RemoveType .php .phtml .php3 .php4 .php5 .php7 .php8 .pht .phar
        <FilesMatch "(?i)\.(php|phtml|php[3-8]|pht|phar|cgi|pl|py|sh|asp|aspx|htaccess)$">
          Require all denied
        </FilesMatch>
        HTACCESS;
